...
RemoteTextMode::RemoteTextMode(TFTHandler& tftRef, Hardware& hwRef)
    : Mode(tftRef), hw(hwRef) 
{
  Serial.println("RemoteTextMode constructed - Using Global Canvas");
}

void RemoteTextMode::enter() {
  // Clear once; update() handles the frame-by-frame clearing
  tft.clearScreen();
}

void RemoteTextMode::update() {
  ...
  // 4. Data Extraction from unified hardware state
  ...
  const int startX = 10;
  const int startY = 24;
  const int lineSpacing = 11; // Adjusted for readability
  int lineNum = 0;

  // Helper to print lines directly to the canvas
  ...
  // 7. [CRITICAL] Push the finished frame to the display
  tft.updateDisplay();
}

void RemoteTextMode::exit() {
